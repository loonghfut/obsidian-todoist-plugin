...
export const QueryRoot: React.FC<Props> = ({ query, warnings }) => {
  ...
  const appendRenderedTasks = useCallback(async () => {
    ...
    const template = settings.appendRenderedTasksTemplate?.trim().length
      ...
      : "{{task}}";
    const markdown = buildRenderedTasksMarkdown(result.tasks, query, template); // Use the imported applyTaskTemplate
...
const buildListMarkdown = (
  ...
  const lines: string[] = [];
  const now = new Date();
  appendTreeLines(lines, trees, 0, taskTemplate, now); // Use the imported applyTaskTemplate
  return lines.join("\n");
};
